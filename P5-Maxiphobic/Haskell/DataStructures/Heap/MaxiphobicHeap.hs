...
merge h@(Node x sz lh rh) h'@(Node x' sz' lh' rh')
 | x < x'          = Node x (sz+sz') mayor menores
 | otherwise       = merge h' h
   where
     mayor
       | size lh >= size rh && size lh >= size  h' = lh
       | size rh >= size lh && size rh >= size h' = rh
       | otherwise = h'
     menores 
       | size lh >= size rh && size lh >= size h' = merge rh h'
       | size rh >= size lh && size rh >= size h' = merge lh h'
