...
intersection (Node x ox s) (Node y oy t) | x < y  = intersection s (Node y oy t)
                                         | x == y && (ox >= oy) = Node x oy (intersection s t)
                                         | x == y && (oy > ox)  = Node x ox (intersection s t)
...
Es más eficiente la implementación ordenada, puesto que en el caso desordenado las operaciones de inserción, eliminación y ocurrencias
siempre serán funciones de complejidad lineal, puesto que hay que comprobar todos los elementos para asegurarnos que no estén al final,
sin embargo con la implementación ordenada podremos hacer distinción entre el mejor y el peor caso, en el peor caso seguirá
siendo lineal, pero en el mejor caso podría llegar a ser de complejidad constante.
