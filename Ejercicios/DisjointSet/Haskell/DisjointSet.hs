...
root x ds@(DS dict) | isElem x ds = root' x ds
                    | otherwise = Nothing
                      where
                        root' x ds | x == (fromJust (D.valueOf x dict)) = Just x 
                                   | otherwise = root' (fromJust (D.valueOf x dict)) ds

-- | Exercise 6. isRoot

isRoot :: Ord a => a -> DisjointSet a -> Bool
isRoot x ds@(DS dict) = x == fromJust (root x ds)

-- | Exercise 7. areConnected

areConnected :: Ord a => a -> a -> DisjointSet a -> Bool
areConnected x y ds@(DS dict) = root x ds == root y ds

-- | Exercise 8. kind

kind :: Ord a => a -> DisjointSet a -> [a]
kind x ds@(DS dict) | isElem x ds = [y | y <- (D.keys dict), areConnected x y ds]
                    | otherwise = []

-- | Exercise 9. union

union :: Ord a => a -> a -> DisjointSet a -> DisjointSet a
union x y ds@(DS dict) | not((isElem x ds) && (isElem y ds)) = error ("Missing Elements from DS")
                       | rootx > rooty = DS (D.insert rootx rooty (D.delete rootx dict))
                       | otherwise     = DS (D.insert rooty rootx (D.delete rooty dict))
                           where
                             rootx = fromJust(root x ds)
                             rooty = fromJust(root y ds)
                             
-- |------------------------------------------------------------------------

flatten :: Ord a => DisjointSet a -> DisjointSet a
flatten ds@(DS dict) = flatten' ds (D.keys dict)
    where flatten' ds []     = ds
          flatten' ds (x:xs) = flatten' (DS (D.insert x rootx (D.delete x dict))) xs 
            where
              rootx = fromJust(root x ds)
