...
weights x =  weights' x D.empty
  where
    weights' [] dict = dict
    weights' (x:xs) dict | D.isDefinedAt x dict = weights' xs dict
                         | otherwise            = weights' xs (D.insert x ((freq x xs)+1) dict) 
                           where
                             freq x [] = 0
...
weights = foldr (\x d ->D.updateOrInsert x (+1) 1 d ) D.empty

2º Usando UpdateorInsert:
...
huffmanLeaves st = huffmanLeaves' (D.keysValues (weights st)) PQ.empty
  where
    huffmanLeaves' [] pq         = pq
    huffmanLeaves' ((k,v):ys) pq = huffmanLeaves' ys (PQ.enqueue (WLeaf k v) pq) 

{-

Implementación mas eficiente usando foldr

huffmanLeaves = (foldr PQ.enqueue PQ.empty) .(map (\(k,v)->WLeaf k v)).(D.keysValues . weights)
...
huffmanTree st | D.size (weights st) < 2 = error "Huffman Tree: The String must have at least two different symbols"
               | otherwise               = huffmanTree' (huffmanLeaves st)
                  where 
                    huffmanTree' pq = mergeAll (merge (PQ.first pq) (PQ.first (PQ.dequeue pq))) (PQ.dequeue (PQ.dequeue pq))
                      where
                        mergeAll wlt pq | PQ.isEmpty pq = wlt
                                        | otherwise     = mergeAll (merge wlt (PQ.first pq)) (PQ.dequeue pq)
...
joinDics dc1 dc2 = joinDics' (D.keysValues dc1) (D.keysValues dc2) D.empty
  where 
    joinDics'[] [] dict          = dict
    joinDics' ((k,v):xs) [] dict = joinDics' xs [] (D.insert k v dict)
    joinDics' [] ((k,v):ys) dict = joinDics' [] ys (D.insert k v dict)
    joinDics' dict1@((k1,v1):xs) dict2@((k2,v2):ys) dict | k1 < k2   = joinDics' xs dict2 (D.insert k1 v1 dict)
                                                         | otherwise = joinDics' dict1 ys (D.insert k2 v2 dict)
...
prefixWith x dict = prefixWith' x (D.keysValues dict) D.empty
  where
    prefixWith' x [] d = d 
    prefixWith' x ((k,v):xs) d = prefixWith' x xs (D.insert k (x:v) d)
...
huffmanCode wlt = huffmanCode' wlt D.empty
  where
    huffmanCode' (WLeaf x n) dict         = D.insert x [] dict
    huffmanCode' (WNode wltl wltr n) dict = joinDics (prefixWith 0 (huffmanCode' wltl dict)) (prefixWith 1 (huffmanCode' wltr dict))
...
encode str dict = encode' str (D.keysValues dict)
  where
    encode' [] keys = []
    encode' (s:str) keys = setCode s keys ++ encode' str keys
      where
        setCode s ((k,v):xs) | s == k = v 
                             | otherwise = setCode s xs
{-
...
takeSymbol x (WLeaf a n) = (a,x)
takeSymbol (x:xs) (WNode wltl wltr n) | x == 0    = takeSymbol xs wltl
                                      | otherwise = takeSymbol xs wltr
...
decode [] _ = ""
decode xs wlt = a : decode b wlt
  where
     (a,b) = takeSymbol xs wlt
